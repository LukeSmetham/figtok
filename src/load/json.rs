...
impl Loader for JsonLoader {
    ...
    fn load(&mut self, entry_path: &String) {
		let mode = match entry_path.ends_with(".json") {
		...
		let (token_sets, themes) = match mode {
			...
			FileMode::MultiFile => {
				...
				let metadata: HashMap<String, Vec<String>> = match serde_json::from_str(&read_file(&format!("{}/$metadata.json", entry_path)).unwrap()) {
					Ok(json) => json,
        ...
        self.parse_token_sets(token_sets);
        self.parse_themes(themes);
    }
...
impl JsonLoader {
	fn parse_token_sets(&mut self, token_sets: HashMap<String, TokenSet>) {
        ...
        for (slug, data) in token_sets {
			...
			// Parse the token set
			let _ = &self.parse_token_set(&slug.to_string(), data, Some(&mut prefix));
		}
    }

    fn parse_themes(&mut self, themes: Vec<serde_json::Value>) {
