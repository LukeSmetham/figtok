    ...
    let (token_sets, themes) = match mode {
        FileMode::SingleFile => {
            let data: serde_json::Value =
                match serde_json::from_str(&read_file(&ctx.entry_path).unwrap()) {
                    Ok(json) => json,
            ...
            let mut token_sets: HashMap<String, HashMap<String, serde_json::Value>> =
                HashMap::new();

            for slug in serde_json::from_value::<Vec<String>>(
                metadata.get("tokenSetOrder").unwrap().to_owned(),
            )
            .unwrap()
            {
        ...
        FileMode::MultiFile => {
            ...
            let themes: Vec<serde_json::Value> = match serde_json::from_str(
                &read_file(&format!("{}/$themes.json", ctx.entry_path)).unwrap(),
            ) {
                Ok(themes) => themes,
